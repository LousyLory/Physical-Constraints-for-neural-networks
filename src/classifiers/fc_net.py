...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
			   ...
			   spatial_update=False, spatial_update_type='L1',
			   mixing_param=0.0,
			   correlate_update=False):
	"""
	...
	self.dtype = dtype
	self.params = {}
	self.spatial_update = spatial_update
	self.correlate_update = correlate_update
	self.corr_multipier = 10
	if self.spatial_update:
		self.spatial_update_type = spatial_update_type
		...
		if self.spatial_update_type == 'L1':
			L1_dist_mat = cx+cy
			self.L1_arr = np.asarray(L1_dist_mat).reshape(-1)
		elif self.spatial_update_type == 'L2':
	...
	if self.correlate_update:
		self.correlate_vector = self.corr_multipier*np.ones(hidden_dims[0])#np.random.random(hidden_dims[0])
		pass
